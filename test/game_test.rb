require "./test/test_helper"

class GameTest < Minitest::Test

  def setup
    @game = Game.new
  end


  def test_game_exists
    assert_instance_of Game, @game
  end


  def test_game_has_game_id
    assert_equal 2012030167
  end


  def test_game_has_season
    assert_equal 20122013 , @game
  end


  def test_game_has_type
    assert_equal "P" , @game
  end


  def test_game_has_date_time
    assert_equal 2013-05-13 , @game
  end


  def test_game_has_away_team_id
    assert_equal 17 , @game
  end


  def test_game_has_home_team_id
    assert_equal 24 , @game
  end


  def test_game_has_away_goals
    assert_equal 3 , @game
  end


  def test_game_has_home_goals
    assert_equal 2, @game
  end


  def test_game_has_outcome
    assert_equal "away win REG" , @game
  end


  def test_game_has_home_rink_side_start
    assert_equal "left" , @game
  end


  def test_game_has_venue
    assert_equal "Honda Center" , @game
  end


  def test_game_has_venue_link
    assert_equal "/api/v1/venues/null" , @game
  end


  def test_game_has_venue_time_zone_id
    assert_equal "America/Los_Angeles" , @game
  end


  def test_game_has_venue_time_zone_offset
    assert_equal -7 , @game
  end


  def test_game_has_venue_time_zone_tz
    assert_equal "PDT" , @game
  end
end
